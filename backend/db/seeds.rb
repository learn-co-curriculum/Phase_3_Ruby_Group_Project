...
events["_embedded"]["events"].each do |event|
    ...
    # attr_id = ""
    # Attraction.all.each_with_index do |attraction, i|
    #     attraction["name"] == event["_embedded"]["attractions"][0]["name"] ? attr_id = i:nil
    # end

    # ven_id = ""
    # Venue.all.each_with_index do |venue, ind|
    #     venue["name"] == event["_embedded"]["venues"][0]["name"] ? ven_id = ind:nil
    # end
